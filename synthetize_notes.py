...
import os
import wave
def save_wav_multi(FILE_NAME, TOP_NOTES=5, sampling_rate=16000, FFT_WINDOW_SECONDS=0.05, amplitude=1000):
    columns = ["time"]
    for n in range(TOP_NOTES):
        columns.append(f"note{n}")
        columns.append(f"anote{n}")
    # Read CSV file
    with open(f"data/output/{FILE_NAME}.csv") as csv_file:
    ...
    for note in notes:
        ...
        for i in range(len(freqs)):
            ...
            samplevoice[i].extend(amplitude * np.sign(np.sin(2 * np.pi * freqs[i] * time)))
        sine_wave*=amplitude/amps[0]
        samples.extend(sine_wave)

    try:
        os.mkdir(f"data/output/{FILE_NAME}")
    except:
        pass

    wave_file = wave.open(f"data/output/{FILE_NAME}/4voices_synthetize.wav", 'wb')
    ...
    for i in range(len(samplevoice)):
        wave_file = wave.open(f"data/output/{FILE_NAME}/4voices_synthetize{i}.wav", 'wb')
        wave_file.setnchannels(1)
        wave_file.setsampwidth(2)
        wave_file.setframerate(sampling_rate)
